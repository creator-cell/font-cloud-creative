...
import type { BrandVoiceSummary, ProjectSummary, ProviderModelSummary } from "@/lib/api/endpoints";
...
export const GenerateForm = ({ token, models, projects, brandVoices, defaults }: { token: string; models: ProviderModelSummary[]; projects: ProjectSummary[]; brandVoices: BrandVoiceSummary[]; defaults?: { provider?: string; model?: string } }) => {
  const [selection, setSelection] = useState<{ provider: string; model: string } | undefined>(
    defaults?.provider && defaults?.model ? { provider: defaults.provider, model: defaults.model } : undefined
  );
        ...
        {result && (
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {result.watermark && (
                <p className="text-xs text-slate-500">Watermark: {result.watermark}</p>
              )}
              <Button
                type="button"
                variant="secondary"
                onClick={() => navigator.clipboard.writeText(JSON.stringify(result.output, null, 2))}
              >
                Copy JSON
              </Button>
            </CardContent>
